#!/bin/bash

# Wonderful Toolchain Installer
# Usage: curl -fsSL https://bread.codes/scripts/wonderful.sh | bash

echo "step 1: detecting architecture..."

# Detect architecture
ARCH=$(uname -m)
case "$ARCH" in
    x86_64)
        BOOTSTRAP_URL="https://wonderful.asie.pl/bootstrap/wf-bootstrap-x86_64.tar.gz"
        ;;
    aarch64|arm64)
        BOOTSTRAP_URL="https://wonderful.asie.pl/bootstrap/wf-bootstrap-aarch64.tar.gz"
        ;;
    *)
        echo "Error: Unsupported architecture: $ARCH"
        exit 1
        ;;
esac

echo "step 2: preparing /opt/wonderful..."

if [ ! -d "/opt/wonderful" ]; then
    if ! sudo mkdir -p /opt/wonderful; then
        echo "Error: Failed to create /opt/wonderful directory"
        exit 2
    fi
fi

echo "step 3: setting ownership..."

if ! sudo chown -R "$USER" /opt/wonderful; then
    echo "Error: Failed to change ownership of /opt/wonderful"
    exit 3
fi

echo "step 4: downloading bootstrap..."

TEMP_FILE=$(mktemp)
if [ -z "$TEMP_FILE" ]; then
    echo "Error: Failed to create temporary file"
    exit 4
fi

echo "step 5: preparing cleanup..."

trap "rm -f $TEMP_FILE" EXIT

echo "step 6: fetching bootstrap..."

if command -v curl &> /dev/null; then
    if ! curl -fsSL "$BOOTSTRAP_URL" -o "$TEMP_FILE"; then
        echo "Error: Failed to download bootstrap"
        exit 5
    fi
elif command -v wget &> /dev/null; then
    if ! wget -q "$BOOTSTRAP_URL" -O "$TEMP_FILE"; then
        echo "Error: Failed to download bootstrap"
        exit 5
    fi
else
    echo "Error: Neither curl nor wget found"
    exit 6
fi

echo "step 7: extracting bootstrap..."

if ! cd /opt/wonderful/; then
    echo "Error: Failed to change to /opt/wonderful directory"
    exit 7
fi

echo "step 8: unpacking files..."

if ! tar xzf "$TEMP_FILE"; then
    echo "Error: Failed to extract bootstrap"
    exit 8
fi

echo "step 9: installing wf-tools..."

export PATH=/opt/wonderful/bin:$PATH

if ! /opt/wonderful/bin/wf-pacman -Syu wf-tools; then
    echo "Error: Failed to install wf-tools"
    exit 9
fi

echo "step 10: enabling BlockSDS repository..."

if ! /opt/wonderful/bin/wf-config repo enable blocksds; then
    echo "Error: Failed to enable BlockSDS repository"
    exit 10
fi

echo "step 11: updating packages..."

if ! /opt/wonderful/bin/wf-pacman -Syu; then
    echo "Error: Failed to update packages"
    exit 11
fi

echo "step 12: installing BlockSDS toolchain..."

if ! /opt/wonderful/bin/wf-pacman -S blocksds-toolchain; then
    echo "Error: Failed to install BlockSDS toolchain"
    exit 12
fi

echo "step 13: installing GBA toolchain..."

if ! /opt/wonderful/bin/wf-pacman -Syu target-gba; then
    echo "Error: Failed to install GBA toolchain"
    exit 13
fi

echo "step 14: finalizing setup..."

source /opt/wonderful/bin/wf-env

echo "Wonderful toolchain installed successfully."
