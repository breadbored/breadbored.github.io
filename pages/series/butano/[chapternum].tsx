...
export const getStaticPaths: GetStaticPaths = async () => {
    ...
    const butanoPosts = posts.filter((post) => post.slug.startsWith('butano-series-'));

    return {
        paths: butanoPosts.map((post) => {
            const chapternum = post.slug.replace('butano-series-', '');
            return {
                params: { chapternum },
            };
        }),
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
    const posts = await getAllPublishedPosts();
    const slug = `butano-series-${params?.chapternum}`;
